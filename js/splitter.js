 ...
 $.fn.splitter = function(args){
	...
	return this.each(function() {
		...
		var focuser = $('<a href="javascript:void(0)"></a>')
			...
			.bind((BROWSER_IS_OPERA?"click":"focus")+opts.eventNamespace, 
		...
		$.each([A,B], function(){
			...
			this._init = opts["size"+this._pane]===true ?
				parseInt($(this[0]).css(opts.split),10) : opts["size"+this._pane];
		});
		...
		if ( opts.resizeTo ) {
            ...
            // hack to prevent infinite loop /jm
            var resizing = false;
			...
			$(window).bind("resize"+opts.eventNamespace, function(){
                ...
                if(!resizing) {
                    resizing = true;
                    if ( !BROWSER_IS_MSIE ) splitter.trigger("resize");
                    resizing = false;
                }
			}).trigger("resize"+opts.eventNamespace);
		}
		else if ( opts.resizeToWidth && !BROWSER_IS_MSIE ) {
			$(window).bind("resize"+opts.eventNamespace, function(){
		...
		if ( opts.dock ) {
			...
			if ( opts.dockKey )
				$('<a title="'+opts.splitbarClass+' toggle dock" href="javascript:void(0)"></a>')
					...
					.bind(BROWSER_IS_OPERA?"click":"focus", function(){ 
