...
function renderComponent(design, component) {
   ...
   //detecting composite glyphs and coloring composite parts  and their subpart labels
    ...
    var box = design.surface.rect(
        //ToDo for later if there is more than just one interaction 300 might need to be changed some thing * number of interactions
        Rect.width(boundingBox), Rect.height(boundingBox) + 300);
    
    //store the height of component bounding bounding box
    design.geom.componentBoxHeight = box.bbox().height;
    //design.geom.componentBoxHeight = Rect.height(boundingBox);
