...
function getDisplayListSegment(componentDefinition, config, share) {
    ...
    //this array stores bioPax term for all non-DNA glyphs
    var nonDNATypesArray = []

    types.forEach((type) => {
        
        //first make sure the bio pax term has the right format and then extract the string followed by "#" 
        /*if ( type._string.split("#").length === 2) {
            
            if ( type._string.split("#")[1] !== 'DnaRegion' ) {
              
              nonDNATypesArray.push(nonDNATypes[type._string.split("#")[1]]);
          }
        }*/
        if ( type._parts.fragment !== 'DnaRegion') {
           nonDNATypesArray.push(nonDNATypes[type._parts.fragment]);
    ...
    if (componentDefinition.sequenceAnnotations.length === 0) {
        ...
        if (componentDefinition.description) tooltip += 'Description: ' + componentDefinition.description + '\n'
         ...
         //right now it only works for modules having only one non-DNA-type   
         if (nonDNATypesArray.length === 1) {
              glyph = nonDNATypesArray[0]
         }  
