...
function DisplayList(displayListObject) {
   ...
   */
   if(displayListObject.interactions !== undefined) {
        ...
        this.combinedInteractions = DisplayList.combineInteractions(displayList, displayListObject.interactions)
...
*******************************************************************************************
*/
DisplayList.combineInteractions = function combineInteractions(displayList, interactions) {

        combinedInteractionsDict = {}
        combinedInteractionsList = []
        nonDNAInteractions = []
        degraded_NonDNAs = []
        for(interaction of interactions) {

            if (interaction.SBO === "SBO:0000179") {
                degraded_NonDNAs.push(interaction.participants[0].segment.name)
            }

            if (interaction.SBO === "SBO:0000177") {
                nonDNAInteractions.push(interaction)
            }
            ...
            if (AvailableInteractions.indexOf(interaction.SBO) !== -1) {
              ...
              for (participant of interaction.participants) {
                   ...
                   if (participant.SO) {
                      ...
                      if (participant.segment.name in combinedInteractionsDict) {

                         combinedInteractionsDict[participant.segment.name]['interactions'].push(interaction)
                         ...
                         combinedInteractionsDict[participant.segment.name] = segmentInfoDict
        ...
        for (key in combinedInteractionsDict) {
            console.log(combinedInteractionsDict)
            for (interaction of combinedInteractionsDict[key]['interactions']) {
                for (participant of interaction.participants) {
                    if (!participant.SO && degraded_NonDNAs.indexOf(participant.segment.name) != -1) {
        ...
        for (interaction of nonDNAInteractions) {
