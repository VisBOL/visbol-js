...
var Rect = require('./geom/rect'),
    ...
    ReactantArrows = require('../font/sbolv/reactantArrow'),
...
function renderSegment(design, segment) {
    ...
    var glyphs = segment.sequence.map(function(glyphObject) {
          ...
          if (segment.role === "reactantProduct") {
            console.log("glyphObject.type",glyphObject.type);
        ...
        */
        ...
        if (segment.role === "reactant1") {
           design.geom.secondReactantOffset = group.bbox().width + design.geom.glyphPadding;
        }

        if (segment.role === "reactantProduct") {
          var arrowsProps = ReactantArrows.render(design, glyphObject, boundingBoxSize );
          var arrowsGlyph = arrowsProps.glyph;
          group.add(arrowsGlyph);
        }
