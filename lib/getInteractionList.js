          ...
          //This is for when there are more than just one of this type of interaction 
           ...
           interaction.participations.forEach(function(participation) {
             ...
             //Attaching type when participant is DNA 
                 ...
                 /*var SOCode = null;
                 //attaching type
                 let components = participation.participant.definition.components;

                 for (let i in components) {
                   
                   if (participation._displayId === components[i]._definition.displayId) {
                      SOCode = components[i]._definition._roles[0]._parts.path.split("/").pop(); 
                      participantObj.SO = components[i]._definition._roles[0]._parts.path.split("/").pop();
                      participantObj.type = soToGlyphType(participantObj.SO); 
                   }
                   else {
                     const displayId = participation._displayId.split("_");
                     if (displayId[0] === components[i]._definition.displayId) {
                       participantObj.SO = components[i]._definition._roles[0]._parts.path.split("/").pop();
                       participantObj.type = soToGlyphType(participantObj.SO); 
                     } 
                   }
                 }
                 if (!participantObj.SO) {
                    participantObj.displayId = participation._displayId;
